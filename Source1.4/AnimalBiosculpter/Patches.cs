...
namespace AnimalBiosculpter.Patches
{
        ...
        public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            if (insertionIndex != -1)
            {
                ...
                var ItI = new List<CodeInstruction>
                {
                    ...
                    new CodeInstruction(OpCodes.Ldfld, AccessTools.Field(typeof(CompBiosculpterPod), nameof(CompBiosculpterPod.biotunedTo))),
    ...
    {
        static bool Prefix(CompBiosculpterPod __instance)
        {
            Pawn occupant = __instance.Occupant;
            CompBiosculpterPod_Cycle currentCycle = __instance.CurrentCycle;
            if (!occupant.RaceProps.Animal)
                __instance.SetBiotuned(occupant);
            currentCycle.CycleCompleted(occupant);
            __instance.EjectContents(interrupted: false, playSounds: true);
            if (occupant != null)
            {
                ...
                if (!occupant.RaceProps.Animal && currentCycle.Props.gainThoughtOnCompletion != null)
                {
